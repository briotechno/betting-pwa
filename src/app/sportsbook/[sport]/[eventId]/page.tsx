...
const MatchTable = ({ match }: { match: any }) => {
  ...
  return (
    <div className="bg-white rounded-b-[12px] shadow-sm border border-[#f36c21] mb-6 mt-4 relative">
      {/* Live Badge - Overlapping Corner */}
      <div className="absolute -top-[10px] -left-[4px] bg-[#28a745] text-white text-[9px] text-normal lg:text-[11px] font-black px-2.5 py-[3px] rounded-[6px] italic leading-tight uppercase z-30 shadow-md transform  transition-transform duration-200 cursor-default flex items-center gap-1 border border-[#238a3a]">
          ...
          <div className="flex-1 flex justify-center">
            {subTabs.map((tab) => (
              <button
                ...
                className={`px-4 h-full text-[11px] font-black uppercase tracking-tight relative ${activeSubTab === tab ? 'text-[#e8612c]' : 'text-gray-400'
                  }`}
              >
                {tab}
                {activeSubTab === tab && (
